...
pub(super) fn create_generated_item_at_playhead(
    area: &gtk::GLArea,
    ...
    if track
        .items
        .iter()
        .any(|item| start >= item.start && start < item.end)
    {
        return;
    }
    ...
    let mut item = match kind {
        GeneratedItemKind::Text => crate::project::VideoItem::text_item(canvas_size, start, end),
        GeneratedItemKind::Shape => crate::project::VideoItem::shape_item(canvas_size, start, end),
        GeneratedItemKind::Paint => crate::project::VideoItem::paint_item(canvas_size, start, end),
        GeneratedItemKind::Background => {
            crate::project::VideoItem::background_item(canvas_size, start, end)
        }
        GeneratedItemKind::Scene3d => {
            crate::project::VideoItem::obj_scene_item(canvas_size, start, end)
        }
    };
    if let VideoItemContent::Text(text) = &mut item.content {
        text.font_families = vec![runtime.borrow().default_text_font_family.clone()];
    }
    ...
    crate::project::commit_edit(&project_state, "create-generated-item");
    drop(project_state);

    crate::selection_state::set_selected_items(selection_state, vec![selected], Some(selected));
...
pub(super) fn create_tts_item_at_playhead(
    area: &gtk::GLArea,
    project: &Rc<RefCell<Project>>,
    player_state: &SharedPlayerState,
    selection_state: &SharedSelectionState,
    ...
    let item_index = items::insert_sorted(
        &mut track.items,
        crate::project::AudioItem::builder(start, end)
            .source(crate::project::AudioSource::Tts(Box::default()))
            .build(),
    );
    let selected = ItemKey {
    ...
    let duration = project_state.duration();
    crate::project::commit_edit(&project_state, "create-tts-item");
    drop(project_state);
...
pub(super) fn create_video_generation_item_at_playhead(
    area: &gtk::GLArea,
    project: &Rc<RefCell<Project>>,
    player_state: &SharedPlayerState,
    selection_state: &SharedSelectionState,
    ...
    let item_index = items::insert_sorted(
        &mut track.items,
        crate::project::VideoItem::video_generation_item(canvas_size, start, end),
    );
    let selected = ItemKey {
        kind: TrackKind::Video,
        track_index: key.track_index,
    ...
    let duration = project_state.duration();
    crate::project::commit_edit(&project_state, "create-video-generation-item");
    drop(project_state);
...
pub(super) fn create_audio_generator_item_at_playhead(
    area: &gtk::GLArea,
    project: &Rc<RefCell<Project>>,
    player_state: &SharedPlayerState,
    selection_state: &SharedSelectionState,
    ...
    let mut item = crate::project::AudioItem::builder(start, end)
        .source(crate::project::AudioSource::Generator(Box::default()))
        .build();
    ...
    let duration = project_state.duration();
    crate::project::commit_edit(&project_state, "create-audio-generator");
    drop(project_state);
