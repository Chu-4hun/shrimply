...
from .audio_features import decode_audio, mel_spectrogram, speaker_features
from .emotion_text import load_emotion_text_analyzer
from .gpt import GenerationOptions
from .model_loader import IndexModels
from .text import GlossaryTerm
from .timing import (
...
_SAMPLE_RATE = 22_050
_HOP_LENGTH = 256
_NATURAL_FRAMES_PER_TOKEN = Fraction(43, 25)
...
def synthesize(
    ...
    tokens = models.tokenizer.tokenize(request.text, glossary)
    segments = models.tokenizer.split_segments(
        tokens, request.max_text_tokens_per_segment
    )
    if not segments:
...
@lru_cache(maxsize=4)
def _reference_features(models: IndexModels, audio: bytes) -> ReferenceFeatures:
    ...
    _, semantic_codes = models.semantic_codec.quantize(conditioning)
    prompt = models.acoustic.regulate_length(semantic_codes, target_length)
...
@lru_cache(maxsize=4)
def _emotion_audio_conditioning(models: IndexModels, audio: bytes) -> Tensor:
    waveform = decode_audio(audio)
    return _semantic_conditioning(models, waveform.samples)


def _semantic_conditioning(models: IndexModels, audio_24khz: Tensor) -> Tensor:
    ...
    output = models.semantic_model(
        input_features=input_features.to(device=models.device, dtype=models.dtype),
    ...
    match request.emotion:
        ...
        case TextEmotion(text=text, strength=strength, randomize=randomize):
            analyzer = load_emotion_text_analyzer(
                models.emotion_text_model, models.device, models.dtype
            )
            values = analyzer.analyze(text or request.text)
    normalized = _normalize_emotion_factors(values)
    scaled = [value * strength for value in normalized]
    vector = _prototype_emotion_vector(models, reference.style, base, scaled, randomize)
    return EmotionCondition(reference.conditioning, vector)


def _normalize_emotion_factors(values: list[float]) -> list[float]:
    if len(values) != len(_EMOTION_BIASES):
        raise ValueError("IndexTTS requires eight emotion factors")
    adjusted = [
        max(0.0, value) * bias
        for value, bias in zip(values, _EMOTION_BIASES, strict=True)
    ]
    total = sum(adjusted)
    return [value * 0.8 / total for value in adjusted] if total > 0.8 else adjusted


def _prototype_emotion_vector(
    models: IndexModels,
    style: Tensor,
    base: Tensor,
    factors: list[float],
    ...
    return vector + (1 - sum(factors)) * base
...
def _semantic_embeddings(
    ...
    token_ids = models.tokenizer.token_ids(segment)
    ...
    codes, speaker_latent = models.gpt.inference_speech(
        reference.conditioning,
        text,
    ...
    code_lengths = torch.tensor([code_length], device=models.device)
    latent = models.gpt(
        speaker_latent,
        text,
        torch.tensor([text.shape[-1]], device=models.device),
        codes,
        code_lengths,
        emotion.conditioning,
        emotion.vector,
    )
    latent = models.acoustic.project_gpt_latent(latent)
    quantized = models.semantic_codec.quantizer.vq2emb(codes.unsqueeze(1))
    embeddings = quantized.transpose(1, 2) + latent
    natural_frames = max(
        1,
        code_length
        * _NATURAL_FRAMES_PER_TOKEN.numerator
        // _NATURAL_FRAMES_PER_TOKEN.denominator,
    )
