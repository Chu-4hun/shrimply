...
struct ManimRenderer {
    sources: HashMap<usize, CachedManimSource>,
    serial: u64,
    renderer: shrimply_manim_wgpu::Renderer,
}
...
impl ManimRenderer {
    fn new() -> Result<Self, String> {
        ...
        let renderer = shrimply_manim_wgpu::Renderer::new()?;
        tracing::info!(
...
fn import_manim_source(
    ...
    shrimply_benchmarking::set_counter("Generated Vulkan / CUDA frames retained", imported_frames);
    shrimply_benchmarking::set_counter("Generated Vulkan / CUDA bytes retained", imported_bytes);
    Ok(ManimCudaSource {
        descriptor,
        array,
...
impl GeneratedGpuRenderer {
    ...
    pub(crate) fn render_manim(
        ...
        if self.manim.is_none() {
            self.manim = Some(ManimRenderer::new()?);
        }
        self.manim
            .as_mut()
