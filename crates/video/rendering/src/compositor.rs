...
use shrimply_project::project::{
    ...
    TransitionSide, VideoItem, VideoSampleMethod, VisualAlphaMask, VisualClipTransition,
...
impl Default for RenderSessions {
    fn default() -> Self {
        Self::new(48_000, false, RenderResourceConfig::default())
    }
}
...
struct ActiveVideoItem<'a> {
    track_index: usize,
    track_id: Uuid,
    item: &'a VideoItem,
    clip_transition: Option<ActiveClipTransition>,
    previous: Option<&'a VideoItem>,
}

#[derive(Clone, Copy)]
struct ActiveClipTransition {
    definition: VisualClipTransition,
    progress: f32,
    role: ClipTransitionRole,
}

#[derive(Clone, Copy, Eq, PartialEq)]
enum ClipTransitionRole {
    Outgoing,
    Incoming,
}

fn active_video_items<'a>(
    track_index: usize,
    track_id: Uuid,
    items: &'a [VideoItem],
    position: Time,
    ...
    for pair in items.windows(2) {
        ...
        let Some(progress) =
            crate::math::clip_transition_progress(outgoing.end, transition.duration, position)
        else {
            continue;
        };
    ...
    timeline_search::overlapping(items, position, position)
        ...
        .map(|(_, item)| ActiveVideoItem {
            track_index,
            track_id,
            item,
            clip_transition: None,
            previous: preload::predecessor(items, item.id),
        })
        .collect()
}
...
fn video_compositor_worker(
    ...
    let mut sessions = RenderSessions::new(48_000, false, resource_config);
