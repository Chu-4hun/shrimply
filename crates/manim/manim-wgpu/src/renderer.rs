...
impl Renderer {
    ...
    pub fn render_rgba_for_validation(
        ...
        let descriptor = self.render_external(0, animation, frame_index)?;
        let pixels = self
            .targets
            .get(&0)
        ...
        Ok(RenderedFrame {
            width: descriptor.descriptor.width,
            height: descriptor.descriptor.height,
            pixels,
        })
    }
    ...
    pub fn render_external(
        ...
        if self.target_descriptor(slot) != Some(descriptor) {
            self.targets
                .insert(slot, Target::new(&self.device, descriptor)?);
        }
        ...
        let target = self
            ...
            .get(&slot)
            .expect("Manim target was initialized");
        let semaphore_value = target.render(
            &self.device,
            &self.queue,
            frame,
            source.camera.background_rgba.to_array(),
        )?;
    ...
    pub fn export_frame(&self, slot: usize) -> Result<ExportedFrame, String> {
        self.targets
            .get(&slot)
            .ok_or_else(|| "Manim render target is not initialized".to_string())?
            .export(&self.device)
    }
...
impl Target {
    ...
    fn render(
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("Manim frame encoder"),
        });
