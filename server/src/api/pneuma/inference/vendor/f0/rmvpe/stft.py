...
class STFT(torch.nn.Module):
    ...
    def forward_basis(self) -> torch.Tensor:
        value = self._buffers["forward_basis"]
        if value is None:
            raise RuntimeError("STFT forward basis is unavailable")
        return value

    @property
    def inverse_basis(self) -> torch.Tensor:
        value = self._buffers["inverse_basis"]
        if value is None:
            raise RuntimeError("STFT inverse basis is unavailable")
        return value

    @property
    def fft_window(self) -> torch.Tensor:
        value = self._buffers["fft_window"]
        if value is None:
            raise RuntimeError("STFT window is unavailable")
        return value
