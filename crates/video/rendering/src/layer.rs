...
impl Visual {
    ...
    pub fn push_motion_blur(
        ...
        let transforms: Rc<[ComposedTransform2D]> = samples
            .into_iter()
...
fn apply_pixel(
    ...
    if state.transform == ComposedTransform2D::IDENTITY
        && state.bounds == ResolvedVisualBounds::default()
        && let GpuFrame::Rgba(layer) = &frame
        && layer.width() == canvas.width.max(1)
        && layer.height() == canvas.height.max(1)
    {
        return Ok((
