...
#[derive(Clone, Copy)]
pub(crate) struct TimelineScale {
    pub(crate) scroll_seconds: f64,
    pub(crate) seconds_per_pixel: f64,
}

impl TimelineScale {
    pub(crate) fn new(scroll_seconds: f64, seconds_per_pixel: f64) -> Self {
        Self {
            scroll_seconds,
            seconds_per_pixel,
        }
    ...
    pub(crate) fn time_to_x(self, time_seconds: f64, x: f64) -> f64 {
        x + (time_seconds - self.scroll_seconds) / self.seconds_per_pixel
    }

    pub(crate) fn frame_width(self, frame_step_seconds: f64) -> f64 {
...
pub(crate) struct RulerStyle {
    pub(crate) height: f64,
    pub(crate) frame_tick_min_width: f64,
    pub(crate) grid_color: Color,
    pub(crate) label_color: Color,
}

pub(crate) struct RulerDraw {
    pub(crate) scale: TimelineScale,
    pub(crate) timeline_x: f64,
...
pub(crate) fn draw(painter: &TimelinePainter, draw: RulerDraw) {
