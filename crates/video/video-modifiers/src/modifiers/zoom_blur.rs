...
pub struct ZoomBlurModifier {
    pub center: TimelineValue<glam::Vec2>,
    pub strength: TimelineValue<f32>,
    pub samples: TimelineValue<f32>,
}

struct ZoomBlurPreview {
    target: PreviewTarget,
    original: ZoomBlurModifier,
    map: glam::Mat3,
    size: glam::Vec2,
    point: glam::Vec2,
    active: bool,
...
impl PreviewProvider for ZoomBlurPreview {
    fn on_draw(
        &mut self,
            ...
            PointerEvent::Cancel => {
                if self.changed {
                    *edits
                        .target_mut(self.target)
                        .downcast_mut::<ZoomBlurModifier>()
                        .expect("zoom blur preview target has wrong type") = self.original.clone();
                }
                self.active = false;
...
impl Default for ZoomBlurModifier {
    fn default() -> Self {
        Self {
            center: TimelineValue::<glam::Vec2>::new_const(glam::Vec2::splat(0.5)),
            strength: TimelineValue::<f32>::new_const(0.2),
            samples: TimelineValue::<f32>::new_const(16.0),
        }
    }
}

impl ModifierModel for ZoomBlurModifier {
    ...
    fn ensure_ids(&mut self, seen: &mut HashSet<Uuid>) {
        ...
        ensure_timeline_value_ids(&mut self.strength, seen);
        ensure_timeline_value_ids(&mut self.samples, seen);
    }

    fn keyframe_span(&self) -> KeyframeSpan {
        combine([
            timeline_value_span(&self.center),
            timeline_value_span(&self.strength),
            timeline_value_span(&self.samples),
        ])
    }

    fn number(&self, id: Uuid) -> Option<&TimelineValue<f32>> {
        [&self.strength, &self.samples]
            .into_iter()
            .find(|value| value.id == id)
    }

    fn number_mut(&mut self, id: Uuid) -> Option<&mut TimelineValue<f32>> {
        [&mut self.strength, &mut self.samples]
            .into_iter()
            .find(|value| value.id == id)
    }

    fn number2(&self, id: Uuid) -> Option<&TimelineValue<glam::Vec2>> {
        (self.center.id == id).then_some(&self.center)
    }

    fn number2_mut(&mut self, id: Uuid) -> Option<&mut TimelineValue<glam::Vec2>> {
        (self.center.id == id).then_some(&mut self.center)
    }
}
