...
use crate::visual_source::GeneratedTransition;
use shrimply_project::project::{TransitionSide, VisualTransitionKind, WriteOrdering};

pub(crate) fn partial_path(path: &Path, progress: f32, reverse: bool) -> Path {
    if progress <= 0.0 {
        return Path::default();
    }
    if progress >= 1.0 {
...
pub(crate) fn contours(path: &Path) -> Vec<Path> {
...
pub(crate) fn morphed_path(from: &Path, to: &Path, progress: f32) -> Path {
    ...
    for index in 0..contour_count {
        ...
        let to_vectors = to_points
            ...
            .map(|point| glam::Vec2::new(point.x, point.y))
            .collect::<Vec<_>>();
        to_points.rotate_left(crate::math::closest_cyclic_shift(
            &from_vectors,
            &to_vectors,
        ));
        ...
        for (point_index, (from, to)) in from_points.into_iter().zip(to_points).enumerate() {
            let point = Point::new(
                crate::math::lerp(from.x, to.x, progress),
                crate::math::lerp(from.y, to.y, progress),
            );
            if point_index == 0 {
...
pub(crate) fn facet_clip(bounds: skia_safe::Rect, index: usize, count: usize) -> Path {
...
pub(crate) fn facet_glint_clip(bounds: skia_safe::Rect, progress: f32) -> Option<Path> {
    ...
    (progress > 0.0 && progress < 1.0).then(|| {
        ...
        let center = crate::math::lerp(bounds.left - width, bounds.right + width, progress);
        Path::polygon(
            &[
...
pub(crate) fn coalesce_mask(bounds: skia_safe::Rect, progress: f32, pool_count: usize) -> Path {
    let mut pools = PathBuilder::new();
    for index in 0..pool_count.clamp(2, 5) {
        let (center, radius) = crate::math::coalesce_pool(progress, index);
        if radius <= 0.0 {
            continue;
        }
        let center = (
...
pub(crate) fn coalesce_filter(extent: f32, softness: f32) -> Option<ImageFilter> {
...
pub(crate) fn soft_refraction_filter(
    progress: f32,
    extent: f32,
    strength: f32,
    texture_scale: f32,
...
pub(crate) fn morphological_filter(
    progress: f32,
    extent: f32,
    amount: f32,
    softness: f32,
...
pub(crate) fn contour_current_paint(progress: f32, width: f32, trail: f32) -> Paint {
    let mut paint = Paint::default();
    paint.set_anti_alias(true);
    paint.set_color(Color::WHITE);
    paint.set_alpha_f(crate::math::transition_accent(progress) * 0.72);
...
pub(crate) fn living_fill_paint(
    bounds: Rect,
    progress: f32,
    band_width: f32,
    softness: f32,
    ...
    let direction = crate::math::polar_degrees(1.0, angle_degrees);
    let span =
    ...
    let center = glam::Vec2::new(bounds.center_x(), bounds.center_y())
        + direction * crate::math::lerp(-span, span, progress.clamp(0.0, 1.0));
    let colors = [
...
pub(crate) fn submobject_progress(
    ...
    let progress =
        crate::math::lagged_transition_progress(transition.progress, index, count, lag_ratio);
    transition
        .interpolation
