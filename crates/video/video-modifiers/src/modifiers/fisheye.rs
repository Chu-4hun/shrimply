...
pub struct FisheyeModifier {
    pub intensity: TimelineValue<f32>,
    pub center: TimelineValue<glam::Vec2>,
}

struct FisheyePreview {
    target: PreviewTarget,
    original: FisheyeModifier,
    map: glam::Mat3,
    size: glam::Vec2,
    point: glam::Vec2,
    active: bool,
...
impl PreviewProvider for FisheyePreview {
    fn on_draw(
        &mut self,
            ...
            PointerEvent::Cancel => {
                if self.changed {
                    *edits
                        .target_mut(self.target)
                        .downcast_mut::<FisheyeModifier>()
                        .expect("fisheye preview target has wrong type") = self.original.clone();
                }
                self.active = false;
...
impl Default for FisheyeModifier {
    fn default() -> Self {
        Self {
            intensity: TimelineValue::<f32>::new_const(0.5),
            center: TimelineValue::<glam::Vec2>::new_const(glam::Vec2::splat(0.5)),
        }
    }
}

impl ModifierModel for FisheyeModifier {
    fn display_name(&self) -> &'static str {
        "Fisheye"
    }

    fn keywords(&self) -> &'static [&'static str] {
        &["lens", "wide angle", "distortion", "warp"]
    }
    ...
    fn number(&self, id: Uuid) -> Option<&TimelineValue<f32>> {
        (self.intensity.id == id).then_some(&self.intensity)
    }

    fn number_mut(&mut self, id: Uuid) -> Option<&mut TimelineValue<f32>> {
        (self.intensity.id == id).then_some(&mut self.intensity)
    }
