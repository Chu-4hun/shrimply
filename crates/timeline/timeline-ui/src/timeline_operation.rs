...
pub(crate) trait TimelineOperationContext {
...
impl SequenceTimeline {
    ...
    pub(crate) fn new(scope: SequenceScopeId) -> Self {
        Self { scope }
    }

    pub(crate) fn for_item(project: &Project, address: &ItemAddress) -> Option<Self> {
        project.item_scope(address).map(Self::new)
    }

    pub(crate) fn for_track(project: &Project, address: &TrackAddress) -> Option<Self> {
