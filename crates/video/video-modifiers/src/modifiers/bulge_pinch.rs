...
impl BulgePinchModifier {
    pub(crate) fn preview_provider(
        ...
        Some(Box::new(BulgePinchPreview {
            target,
            original: self.clone(),
            map,
            size,
...
impl PreviewProvider for BulgePinchPreview {
    fn on_draw(
        &mut self,
            ...
            PointerEvent::Samples { input, .. } if self.active => {
                ...
                let changed = super::preview::set_vec2(
                    &mut edits
                        ...
                        .downcast_mut::<BulgePinchModifier>()
                        .expect("bulge preview target has wrong type")
                        .center,
                    context.local_time(),
                    point,
                );
            ...
            PointerEvent::Cancel => {
                if self.changed {
                    *edits
                        .target_mut(self.target)
                        .downcast_mut::<BulgePinchModifier>()
                        .expect("bulge preview target has wrong type") = self.original.clone();
                }
                self.active = false;
...
impl Default for BulgePinchModifier {
    fn default() -> Self {
        Self {
            center: TimelineValue::<glam::Vec2>::new_const(glam::Vec2::splat(0.5)),
            radius: TimelineValue::<f32>::new_const(0.5),
            strength: TimelineValue::<f32>::new_const(0.5),
        }
    }
}

impl ModifierModel for BulgePinchModifier {
    fn display_name(&self) -> &'static str {
        "Bulge"
    }

    fn keywords(&self) -> &'static [&'static str] {
        &["pinch", "warp", "distortion", "inflate", "squeeze"]
    }
    ...
    fn number_mut(&mut self, id: Uuid) -> Option<&mut TimelineValue<f32>> {
        [&mut self.radius, &mut self.strength]
            .into_iter()
            .find(|value| value.id == id)
    }
