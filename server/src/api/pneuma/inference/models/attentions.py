...
from api.pneuma.inference.models.blocks import LayerNorm, RMSNorm


def _constant_pad_dim(
    x: torch.Tensor,
    dim: int,
    left: int | torch.SymInt = 0,
    right: int | torch.SymInt = 0,
) -> torch.Tensor:
    ...
    if left > 0:
        ...
        left_shape[dim] = int(left)
        parts.append(x.new_zeros(left_shape))
    parts.append(x)
    if right > 0:
        right_shape = list(x.shape)
        right_shape[dim] = int(right)
        parts.append(x.new_zeros(right_shape))
...
class Encoder(nn.Module):
    def __init__(
        ...
        for i in range(self.n_layers):
            ...
            if norm_type == "rms":
                self.norm_layers_1.append(RMSNorm(hidden_channels))
                self.norm_layers_2.append(RMSNorm(hidden_channels))
            else:
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor) -> torch.Tensor:
        ...
        for attn_layers, norm_layers_1, ffn_layers, norm_layers_2 in zippep:
            if self.norm_position == "pre":
                y = norm_layers_1(x)
                y = attn_layers(y, y, attn_mask)
                y = self.drop(y)
                x = x + y

                y = ffn_layers(norm_layers_2(x), x_mask)
                y = self.drop(y)
                x = x + y
            else:
...
class Decoder(nn.Module):
    def __init__(
        ...
        for i in range(self.n_layers):
            ...
            if norm_type == "rms":
                self.norm_layers_0.append(RMSNorm(hidden_channels))
                self.norm_layers_1.append(RMSNorm(hidden_channels))
                self.norm_layers_2.append(RMSNorm(hidden_channels))
            else:
...
class MultiHeadAttention(nn.Module):
    ...
    def _get_relative_embeddings(
        ...
        if pad_length > 0:
            padded_relative_embeddings = _constant_pad_dim(
                relative_embeddings,
                dim=1,
                left=pad_length,
                right=pad_length,
    ...
    def _relative_position_to_absolute_position(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = _constant_pad_dim(x, dim=-1, right=1)

        # Concat extra elements so to add up to shape (len+1, 2*len-1).
        x_flat = x.view((batch, heads, length * 2 * length))
        x_flat = _constant_pad_dim(x_flat, dim=-1, right=length - 1)
    ...
    def _absolute_position_to_relative_position(self, x: torch.Tensor) -> torch.Tensor:
        ...
        batch, heads, length, _ = x.size()
        # padd along column
        x = _constant_pad_dim(x, dim=-1, right=length - 1)
        x_flat = x.view((batch, heads, (length**2) + (length * (length - 1))))
        # add 0's in the beginning that will skew the elements after reshape
        x_flat = _constant_pad_dim(x_flat, dim=-1, left=length)
        x_final = x_flat.view([batch, heads, length, 2 * length])[:, :, :, 1:]
        return x_final

    def _attention_bias_proximal(self, length: int | torch.SymInt) -> torch.Tensor:
        ...
        """
        r = torch.arange(int(length), dtype=torch.float32)
...
class FFN(nn.Module):
    def __init__(
        ...
        if ffn_type == "gated":
            self.conv_gate = nn.Conv1d(in_channels, filter_channels, kernel_size)
            self.conv_up = nn.Conv1d(in_channels, filter_channels, kernel_size)
        else:
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor) -> torch.Tensor:
        padded = self.padding(x, x_mask)
        if self.ffn_type == "gated":
            gate = self.conv_gate(padded)
            gate = self._apply_activation(gate)
            up = self.conv_up(padded)
            x = gate * up
        else:
            x = self.conv_1(padded)
    ...
    def _causal_padding(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.pad(
            x,
            #   utils.convert_pad_shape(padding)
            (pad_l, pad_r, 0, 0, 0, 0),
        )
        return x
