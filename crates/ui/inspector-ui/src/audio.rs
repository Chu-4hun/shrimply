            ...
            audio_items.push(
                DefaultInspectorItem::new(
                    ...
                    |context, settings| {
                        ...
                        **current = settings;
                        shrimply_project::project::commit_edit(&project, "reset-tts");
                        drop(project);
                        (context.refresh)();
                    },
                )
...
fn tts_editor(
    ...
    shrimply_tts_ui::editor(
        ...
        move |settings| {
            ...
            let Some(item) = project.audio_item_mut(&changed_key) else {
                return;
            };
            let AudioSource::Tts(current) = &mut item.source else {
                return;
        ...
        move || {
            shrimply_project::project::commit_edit(&commit_project.borrow(), "edit-tts");
        },
        move |path, source_duration| {
            let mut project = generated_project.borrow_mut();
            let track_address = key.track();
            let item_id = key.item_id();
            let Some(shrimply_project::project::TrackRef::Audio(track)) =
                project.track(&track_address)
            else {
                drop(project);
            ...
            if !matches!(&item.source, AudioSource::Tts(_)) {
                drop(project);
                let _ = std::fs::remove_file(path);
                return;
            }
