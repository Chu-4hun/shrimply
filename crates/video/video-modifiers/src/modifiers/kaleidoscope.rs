...
pub struct KaleidoscopeModifier {
    pub center: TimelineValue<glam::Vec2>,
    pub segments: TimelineValue<f32>,
    pub rotation_degrees: TimelineValue<f32>,
}

struct KaleidoscopePreview {
    target: PreviewTarget,
    original: KaleidoscopeModifier,
    map: glam::Mat3,
    size: glam::Vec2,
    point: glam::Vec2,
    active: bool,
...
impl KaleidoscopeModifier {
    pub(crate) fn preview_provider(
        ...
        Some(Box::new(KaleidoscopePreview {
            target,
            original: self.clone(),
            map,
            size,
...
impl PreviewProvider for KaleidoscopePreview {
    fn on_draw(
        &mut self,
            ...
            PointerEvent::Samples { input, .. } if self.active => {
                ...
                let changed = super::preview::set_vec2(
                    &mut edits
                        ...
                        .downcast_mut::<KaleidoscopeModifier>()
                        .expect("kaleidoscope preview target has wrong type")
                        .center,
                    time,
                    point,
                );
...
impl Default for KaleidoscopeModifier {
    fn default() -> Self {
        Self {
            center: TimelineValue::<glam::Vec2>::new_const(glam::Vec2::splat(0.5)),
            segments: TimelineValue::<f32>::new_const(6.0),
            rotation_degrees: TimelineValue::<f32>::new_const(0.0),
        }
    }
}

impl ModifierModel for KaleidoscopeModifier {
    fn display_name(&self) -> &'static str {
        "Kaleidoscope"
    }

    fn keywords(&self) -> &'static [&'static str] {
        &["symmetry", "mirror", "pattern"]
    }

    fn ensure_ids(&mut self, seen: &mut HashSet<Uuid>) {
        ensure_timeline_value_ids(&mut self.center, seen);
        ensure_timeline_value_ids(&mut self.segments, seen);
    ...
    fn keyframe_span(&self) -> KeyframeSpan {
        combine([
            ...
            timeline_value_span(&self.segments),
            timeline_value_span(&self.rotation_degrees),
        ])
    }

    fn number(&self, id: Uuid) -> Option<&TimelineValue<f32>> {
        [&self.segments, &self.rotation_degrees]
            .into_iter()
            .find(|value| value.id == id)
    }

    fn number_mut(&mut self, id: Uuid) -> Option<&mut TimelineValue<f32>> {
        [&mut self.segments, &mut self.rotation_degrees]
            .into_iter()
            .find(|value| value.id == id)
    }
