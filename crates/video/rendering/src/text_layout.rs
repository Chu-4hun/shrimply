...
impl TextLayout {
    pub(crate) fn color_glyphs(&self, color: Color<u8>) -> Option<ColorGlyphImage> {
        let color = Color {
            a: u8::MAX,
            ..color
        };
...
fn build(
    ...
    let mut word_subpaths = (0..word_count)
        ...
        .collect::<Vec<_>>();
    let mut path = PathBuilder::new();
    for contour in crate::path_transition::contours(&base_path) {
    ...
    if let Some(color_glyphs) = &mut color_glyphs {
        ...
        for contour in crate::path_transition::contours(&silhouette) {
