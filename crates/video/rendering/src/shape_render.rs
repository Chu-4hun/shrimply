...
use shrimply_project::project::{
    CanvasSize, Color, ShapeItem, ShapeKind, ShapeRoundingStrategy, VideoItem,
};
use uuid::Uuid;
...
impl VisualElement for ShapeElement {
    ...
    fn draw(
        ...
        let shrimply_project::project::VideoItemContent::Shape(shape) = &request.item.content
        else {
        ...
        let mut decoration =
            decoration_outset(shape, &evaluation, &mut self.expressions, rotation_degrees);
        ...
        Ok(VisualRender::Ready(Visual::Vector(VectorVisual::prepared(
            Box::new(DeferredShapeFrame {
                ...
                .map_err(|error| format!("serialize shape raster cache key: {error}"))?,
...
impl GeneratedVisual for DeferredShapeFrame {
    fn draw(
        &self,
        canvas: &Canvas,
        evaluation: &TransformEvaluation,
...
impl VisualData for DeferredShapeFrame {
    ...
    fn morph_scene(&self) -> Option<crate::vector_morph::MorphScene> {
        ...
        let path = shape_path(
            &shape,
            Rect::from_xywh(0.0, 0.0, shape.size.x, shape.size.y),
        )
...
fn draw_shape(
    ...
    let shape = resolve_shape_draw(shape_item, rotation, eval, expression_cache);
...
fn resolve_shape_draw(
    ...
    ShapeDraw {
        ...
        shadow_offset: crate::math::rotate_degrees(
...
fn shape_path(shape: &ShapeDraw, rect: Rect) -> Path {
    match shape.shape {
        ...
        ShapeKind::Star => polygon_path(
            crate::math::fit_vertices(
                crate::math::star_vertices(
                    shape.star_points,
        ...
        ShapeKind::Diamond => polygon_path(
            crate::math::fit_vertices(
                crate::math::regular_polygon_vertices(4, -std::f32::consts::FRAC_PI_2),
                glam::Vec2::ZERO,
                shape.size,
            ),
            shape.corner_radius,
        ...
        ShapeKind::Pentagon => polygon_path(
            crate::math::fit_vertices(
                crate::math::regular_polygon_vertices(5, -std::f32::consts::FRAC_PI_2),
                glam::Vec2::ZERO,
                shape.size,
            ),
            shape.corner_radius,
        ...
        ShapeKind::Hexagon => polygon_path(
            crate::math::fit_vertices(
                crate::math::regular_polygon_vertices(6, 0.0),
                glam::Vec2::ZERO,
                shape.size,
            ),
            shape.corner_radius,
        ...
        ShapeKind::Octagon => polygon_path(
            crate::math::fit_vertices(
                crate::math::regular_polygon_vertices(8, std::f32::consts::FRAC_PI_8),
                glam::Vec2::ZERO,
                shape.size,
            ),
            shape.corner_radius,
        ...
        ShapeKind::Cross => polygon_path(
            crate::math::cross_vertices(shape.size, shape.cross_arm_thickness),
            shape.corner_radius,
            shape.rounding_strategy,
        ),
    }
...
fn ellipse_path(shape: &ShapeDraw, completion_degrees: f32) -> Path {
    let Some(segment) = crate::math::ellipse_segment(shape.size, completion_degrees) else {
        return Path::new();
    };
    ...
    match strategy {
        ...
        ShapeRoundingStrategy::Circular => {
            builder.conic_to(
                corner,
                exit,
                crate::math::corner_conic_weight(
    ...
    match transition.kind {
        ...
        shrimply_project::project::VisualTransitionKind::ContourCurrent => {
            draw_shape_path(
                ...
                shape,
                full,
                opacity * crate::math::vector_reveal_opacity(progress),
            );
            canvas.draw_path(
                full,
...
fn polygon_path(
    ...
    let Some(corners) = crate::math::polygon_corners(&vertices, corner_radius, circular) else {
        return sharp_polygon_path(&vertices);
    };
    let mut builder = PathBuilder::new();
