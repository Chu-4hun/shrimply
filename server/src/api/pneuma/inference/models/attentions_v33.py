...
V33_FFN_CHANNELS = 1024
V33_LAYER_SCALE_INIT = 0.01
ROPE_BASE = 10000.0
LOCAL_CONV_KERNEL_SIZE = 7


class V33HeadRMSNorm(nn.Module):
...
class V33RotaryEmbedding(nn.Module):
...
class V33Attention(nn.Module):
    def __init__(self, channels: int, n_heads: int) -> None:
        ...
        self.q_norm = V33HeadRMSNorm(n_heads, self.head_channels)
        self.k_norm = V33HeadRMSNorm(n_heads, self.head_channels)
        self.rope = V33RotaryEmbedding(self.head_channels)
...
class V33SwiGLUFFN(nn.Module):
    def __init__(
        self,
        channels: int,
        kernel_size: int,
        ffn_channels: int = V33_FFN_CHANNELS,
    ) -> None:
        super().__init__()
...
class V33LocalConvBranch(nn.Module):
    def __init__(
        ...
        self.pointwise_up = nn.Conv1d(channels, channels * 4, 1)
        self.pointwise_down = nn.Conv1d(channels * 4, channels, 1)
        self.layer_scale = nn.Parameter(torch.full((channels, 1), V33_LAYER_SCALE_INIT))

    def forward(self, x: torch.Tensor, x_mask: torch.Tensor) -> torch.Tensor:
        y = self.norm(x)
        y = self.depthwise(y * x_mask)
        y = self.pointwise_up(y)
        y = F.gelu(y)
        y = self.pointwise_down(y)
        return y * self.layer_scale.unsqueeze(0) * x_mask


class V33EncoderLayer(nn.Module):
    def __init__(self, channels: int, n_heads: int, kernel_size: int) -> None:
        super().__init__()
        self.attn_norm = RMSNorm(channels)
        self.attn = V33Attention(channels, n_heads)
        self.attn_scale = nn.Parameter(torch.full((channels, 1), V33_LAYER_SCALE_INIT))
        self.local = V33LocalConvBranch(channels)
        self.ffn_norm = RMSNorm(channels)
        self.ffn = V33SwiGLUFFN(channels, kernel_size)
        self.ffn_scale = nn.Parameter(torch.full((channels, 1), V33_LAYER_SCALE_INIT))

    def forward(self, x: torch.Tensor, x_mask: torch.Tensor) -> torch.Tensor:
        x = x + self.attn_scale.unsqueeze(0) * self.attn(self.attn_norm(x), x_mask)
        x = x + self.local(x, x_mask)
        x = x + self.ffn_scale.unsqueeze(0) * self.ffn(self.ffn_norm(x), x_mask)
        return x * x_mask


class V33Encoder(nn.Module):
    def __init__(
        self,
        hidden_channels: int,
        n_heads: int,
        ...
        self.layers = nn.ModuleList(
            [
                V33EncoderLayer(hidden_channels, n_heads, kernel_size)
