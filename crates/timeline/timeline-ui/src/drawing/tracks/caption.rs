...
pub(super) fn draw(input: TrackDrawInput<'_>, first_visible_row: usize, last_visible_row: usize) {
    let TrackDrawInput {
        painter,
        project,
        selected_items,
    ...
    for (track_index, track) in project.caption_tracks.iter().enumerate() {
        let Some(row) =
            visual_row_for_track(project, TrackKind::Caption, track_index, virtual_tracks)
        else {
            continue;
        };
        ...
        draw_selected_track_fill(
            painter,
            selected_tracks,
            &crate::project::TrackAddress::Caption { track_id: track.id },
            timeline_x,
            y,
            timeline_width,
        );
        ...
        for (index, item) in track.items.iter().enumerate() {
            ...
            if !dragged && !resizing {
                if item_width >= MIN_DETAILED_ITEM_WIDTH {
                    draw_caption_item(painter, item, timeline_x, y, view, selected);
        ...
        draw_tiny_item_fill(painter, &tiny_item_columns, timeline_x, y, Color::YELLOW5);
