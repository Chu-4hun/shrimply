...
#[derive(Clone)]
pub(crate) struct MorphPaintLayer {
    pub paint: Paint,
    pub offset: glam::Vec2,
}

#[derive(Clone)]
pub(crate) struct MorphObject {
    pub path: shrimply_math_geometry::MorphPath,
    pub appearance: Vec<MorphPaintLayer>,
}

#[derive(Clone)]
pub(crate) struct MorphScene {
...
pub(crate) struct PreparedVectorMorph {
    source: MorphScene,
    target: MorphScene,
...
pub(crate) struct MorphFrame {
    morph: Rc<PreparedVectorMorph>,
    progress: f32,
}

impl PreparedVectorMorph {
    pub(crate) fn new(source: MorphScene, target: MorphScene) -> Self {
    ...
    pub(crate) fn frame(self: &Rc<Self>, progress: f32) -> MorphFrame {
        MorphFrame {
            morph: Rc::clone(self),
    ...
    pub(crate) fn source(&self) -> &MorphScene {
        &self.source
    }

    pub(crate) fn target(&self) -> &MorphScene {
        &self.target
    }
}
...
pub(crate) fn apply_vector_operations(
...
pub(crate) fn skia_path_to_morph(path: &Path) -> shrimply_math_geometry::MorphPath {
...
pub(crate) fn morph_to_skia_path(path: &shrimply_math_geometry::MorphPath) -> Path {
