...
impl DfTract {
    pub fn new(dfp: DfParams, rp: &RuntimeParams) -> Result<Self> {
        ...
        let enc = enc.into_runnable()?.spawn()?;
        let erb_dec = erb_dec.into_runnable()?.spawn()?;
        let df_dec = df_dec.into_runnable()?.spawn()?;
    ...
    pub fn process_raw(&mut self) -> Result<(f32, Option<Tensor>, Option<Tensor>)> {
        ...
        let mut enc_emb = self.enc.run(tvec!(
            self.erb_buf.clone().into_tvalue(),
            self.cplx_buf
                .clone()
        ...
        let mut lsnr = enc_emb.pop().unwrap().into_tensor();
        let lsnr = *lsnr.to_scalar_mut::<f32>()?;
