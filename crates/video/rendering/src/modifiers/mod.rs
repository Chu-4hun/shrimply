...
use crate::layer::{RasterVisual, VectorVisual, Visual};
...
pub(crate) trait VectorModifierRuntime {
    fn apply_vector(
        &self,
        input: VectorVisual,
...
pub(crate) fn apply(
    ...
    match effect {
        ...
        ModifierEffect::Vector(effect) => {
            let Visual::Vector(input) = input else {
                unreachable!("validated vector modifier chain received raster input")
            };
            apply_vector(effect, input, context).map(Visual::Vector)
        }
        ModifierEffect::Rasterize(effect) => {
            let configured = effect
                .sample_method
                .value_at(context.evaluation.local_time());
            let sample_method = if context.accuracy.content_accurate() {
                configured
            } else {
                if matches!(configured, VideoSampleMethod::Nearest) {
...
fn apply_vector(
    ...
    vector_runtime(effect).apply_vector(input, context)
}
...
fn vector_runtime(effect: &VectorModifierEffect) -> &dyn VectorModifierRuntime {
    match effect {
        VectorModifierEffect::Transform(effect) => &**effect,
        VectorModifierEffect::Repeat(effect) => &**effect,
        VectorModifierEffect::ShakyPath(effect) => &**effect,
        VectorModifierEffect::PathOffset(effect) => &**effect,
        VectorModifierEffect::Opacity(effect) => effect,
        VectorModifierEffect::Hsv(effect) => effect,
        VectorModifierEffect::TextMask(effect) => effect,
    }
}
